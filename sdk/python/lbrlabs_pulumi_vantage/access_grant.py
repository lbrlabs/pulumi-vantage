# coding=utf-8
# *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import copy
import warnings
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
from . import _utilities

__all__ = ['AccessGrantArgs', 'AccessGrant']

@pulumi.input_type
class AccessGrantArgs:
    def __init__(__self__, *,
                 resource_token: pulumi.Input[str],
                 team_token: pulumi.Input[str],
                 access: Optional[pulumi.Input[str]] = None):
        """
        The set of arguments for constructing a AccessGrant resource.
        :param pulumi.Input[str] resource_token: Token of the resource being granted.
        :param pulumi.Input[str] team_token: Token of the team being granted.
        :param pulumi.Input[str] access: Access level of the grant. Must be either `allowed` or `denied`.
        """
        pulumi.set(__self__, "resource_token", resource_token)
        pulumi.set(__self__, "team_token", team_token)
        if access is not None:
            pulumi.set(__self__, "access", access)

    @property
    @pulumi.getter(name="resourceToken")
    def resource_token(self) -> pulumi.Input[str]:
        """
        Token of the resource being granted.
        """
        return pulumi.get(self, "resource_token")

    @resource_token.setter
    def resource_token(self, value: pulumi.Input[str]):
        pulumi.set(self, "resource_token", value)

    @property
    @pulumi.getter(name="teamToken")
    def team_token(self) -> pulumi.Input[str]:
        """
        Token of the team being granted.
        """
        return pulumi.get(self, "team_token")

    @team_token.setter
    def team_token(self, value: pulumi.Input[str]):
        pulumi.set(self, "team_token", value)

    @property
    @pulumi.getter
    def access(self) -> Optional[pulumi.Input[str]]:
        """
        Access level of the grant. Must be either `allowed` or `denied`.
        """
        return pulumi.get(self, "access")

    @access.setter
    def access(self, value: Optional[pulumi.Input[str]]):
        pulumi.set(self, "access", value)


@pulumi.input_type
class _AccessGrantState:
    def __init__(__self__, *,
                 access: Optional[pulumi.Input[str]] = None,
                 resource_token: Optional[pulumi.Input[str]] = None,
                 team_token: Optional[pulumi.Input[str]] = None,
                 token: Optional[pulumi.Input[str]] = None):
        """
        Input properties used for looking up and filtering AccessGrant resources.
        :param pulumi.Input[str] access: Access level of the grant. Must be either `allowed` or `denied`.
        :param pulumi.Input[str] resource_token: Token of the resource being granted.
        :param pulumi.Input[str] team_token: Token of the team being granted.
        :param pulumi.Input[str] token: Token of the access grant.
        """
        if access is not None:
            pulumi.set(__self__, "access", access)
        if resource_token is not None:
            pulumi.set(__self__, "resource_token", resource_token)
        if team_token is not None:
            pulumi.set(__self__, "team_token", team_token)
        if token is not None:
            pulumi.set(__self__, "token", token)

    @property
    @pulumi.getter
    def access(self) -> Optional[pulumi.Input[str]]:
        """
        Access level of the grant. Must be either `allowed` or `denied`.
        """
        return pulumi.get(self, "access")

    @access.setter
    def access(self, value: Optional[pulumi.Input[str]]):
        pulumi.set(self, "access", value)

    @property
    @pulumi.getter(name="resourceToken")
    def resource_token(self) -> Optional[pulumi.Input[str]]:
        """
        Token of the resource being granted.
        """
        return pulumi.get(self, "resource_token")

    @resource_token.setter
    def resource_token(self, value: Optional[pulumi.Input[str]]):
        pulumi.set(self, "resource_token", value)

    @property
    @pulumi.getter(name="teamToken")
    def team_token(self) -> Optional[pulumi.Input[str]]:
        """
        Token of the team being granted.
        """
        return pulumi.get(self, "team_token")

    @team_token.setter
    def team_token(self, value: Optional[pulumi.Input[str]]):
        pulumi.set(self, "team_token", value)

    @property
    @pulumi.getter
    def token(self) -> Optional[pulumi.Input[str]]:
        """
        Token of the access grant.
        """
        return pulumi.get(self, "token")

    @token.setter
    def token(self, value: Optional[pulumi.Input[str]]):
        pulumi.set(self, "token", value)


class AccessGrant(pulumi.CustomResource):
    @overload
    def __init__(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 access: Optional[pulumi.Input[str]] = None,
                 resource_token: Optional[pulumi.Input[str]] = None,
                 team_token: Optional[pulumi.Input[str]] = None,
                 __props__=None):
        """
        Manages an AccessGrant.

        ## Example Usage

        ```python
        import pulumi
        import lbrlabs_pulumi_vantage as vantage

        demo_access_grant = vantage.AccessGrant("demoAccessGrant",
            resource_token="rprt_39d256c871cb6b2b",
            team_token="team_bd5c2d8abc233bfd")
        ```

        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[str] access: Access level of the grant. Must be either `allowed` or `denied`.
        :param pulumi.Input[str] resource_token: Token of the resource being granted.
        :param pulumi.Input[str] team_token: Token of the team being granted.
        """
        ...
    @overload
    def __init__(__self__,
                 resource_name: str,
                 args: AccessGrantArgs,
                 opts: Optional[pulumi.ResourceOptions] = None):
        """
        Manages an AccessGrant.

        ## Example Usage

        ```python
        import pulumi
        import lbrlabs_pulumi_vantage as vantage

        demo_access_grant = vantage.AccessGrant("demoAccessGrant",
            resource_token="rprt_39d256c871cb6b2b",
            team_token="team_bd5c2d8abc233bfd")
        ```

        :param str resource_name: The name of the resource.
        :param AccessGrantArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        ...
    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(AccessGrantArgs, pulumi.ResourceOptions, *args, **kwargs)
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 access: Optional[pulumi.Input[str]] = None,
                 resource_token: Optional[pulumi.Input[str]] = None,
                 team_token: Optional[pulumi.Input[str]] = None,
                 __props__=None):
        opts = pulumi.ResourceOptions.merge(_utilities.get_resource_opts_defaults(), opts)
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError('Expected resource options to be a ResourceOptions instance')
        if opts.id is None:
            if __props__ is not None:
                raise TypeError('__props__ is only valid when passed in combination with a valid opts.id to get an existing resource')
            __props__ = AccessGrantArgs.__new__(AccessGrantArgs)

            __props__.__dict__["access"] = access
            if resource_token is None and not opts.urn:
                raise TypeError("Missing required property 'resource_token'")
            __props__.__dict__["resource_token"] = resource_token
            if team_token is None and not opts.urn:
                raise TypeError("Missing required property 'team_token'")
            __props__.__dict__["team_token"] = team_token
            __props__.__dict__["token"] = None
        super(AccessGrant, __self__).__init__(
            'vantage:index/accessGrant:AccessGrant',
            resource_name,
            __props__,
            opts)

    @staticmethod
    def get(resource_name: str,
            id: pulumi.Input[str],
            opts: Optional[pulumi.ResourceOptions] = None,
            access: Optional[pulumi.Input[str]] = None,
            resource_token: Optional[pulumi.Input[str]] = None,
            team_token: Optional[pulumi.Input[str]] = None,
            token: Optional[pulumi.Input[str]] = None) -> 'AccessGrant':
        """
        Get an existing AccessGrant resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[str] access: Access level of the grant. Must be either `allowed` or `denied`.
        :param pulumi.Input[str] resource_token: Token of the resource being granted.
        :param pulumi.Input[str] team_token: Token of the team being granted.
        :param pulumi.Input[str] token: Token of the access grant.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _AccessGrantState.__new__(_AccessGrantState)

        __props__.__dict__["access"] = access
        __props__.__dict__["resource_token"] = resource_token
        __props__.__dict__["team_token"] = team_token
        __props__.__dict__["token"] = token
        return AccessGrant(resource_name, opts=opts, __props__=__props__)

    @property
    @pulumi.getter
    def access(self) -> pulumi.Output[str]:
        """
        Access level of the grant. Must be either `allowed` or `denied`.
        """
        return pulumi.get(self, "access")

    @property
    @pulumi.getter(name="resourceToken")
    def resource_token(self) -> pulumi.Output[str]:
        """
        Token of the resource being granted.
        """
        return pulumi.get(self, "resource_token")

    @property
    @pulumi.getter(name="teamToken")
    def team_token(self) -> pulumi.Output[str]:
        """
        Token of the team being granted.
        """
        return pulumi.get(self, "team_token")

    @property
    @pulumi.getter
    def token(self) -> pulumi.Output[str]:
        """
        Token of the access grant.
        """
        return pulumi.get(self, "token")

